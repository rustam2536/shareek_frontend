...
const ReportUser = () => {
    ...
    const getData = async (pageNum = 1) => {
        ...
        try {
            showLoader();
            const res = await api.get(`/api/report_user/admin/list?page=${pageNum}&searchTerm=${searchTerm}`);
            if (res.data.success) {
                    ...
                    <div className="col-xl-12">
                        <div className="card">
                            <div className="card-header d-flex justify-content-between align-items-center">
                                <h4 className="card-title text-uppercase">Report User</h4>
                                <div className="d-flex">
                                    <input
                                        type="text"
                                        className="form-control"
                                        placeholder="Search by user Id, reporter Id..."
                                        value={searchInput}
                                            ...
                                            {dataList?.length > 0 ? (
                                                dataList.map((user, index) => (
                                                    <tr key={user.uniqueId}>
                                                        ...
                                                        <td>
                                                            <div>
                                                                <Link class="" to={"/seller?u=" + user.userId}>
                                                                    {user.userId}
                                                                </Link>
                                                            </div>
                                                            <div>
                                                        ...
                                                        <td>
                                                            <div>
                                                                <Link class="" to={"/seller?u=" + user.reporterId}>
                                                                    {user.reporterId}
                                                                </Link>
                                                            </div>
                                                            <div>
                                                        ...
                                                        <td>
                                                            {
                                                                user.chats && (
                                                                    ...
                                                                    <div style={{ cursor: "pointer" }} onClick={() => setModalMessage(user.chats)}>
